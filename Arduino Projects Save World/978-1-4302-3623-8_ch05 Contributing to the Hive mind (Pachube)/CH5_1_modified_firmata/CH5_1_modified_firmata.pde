 ...
 *
 * Note: one bug fix and changed sampling interval / com speed EWP
 */
#include <Firmata.h>
...
void setup()
{
  ...
  Firmata.begin(28800);
  // your own additional setup stuff here
}
...
void loop()
{
  ...
  for (i=0; i<TOTAL_PORTS; i++) {
    sendPort(i, readPort(i, 255));  // bug fix this value specifies sending all port data
  }
  /* make sure that the FTDI buffer doesn't go over 60 bytes, otherwise you
     get long, random delays.  So only read analogs every 20ms or so */
